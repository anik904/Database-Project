...
course = Course_T(courseID='MIS430', courseName="Data Communication and Networking", numOfCredits=4, programID=4,
                  courseType="Core") #change
course.save()

# CO
plolist = list(PLO_T.objects.filter(programID=4))

colist = []

colist.append(CO_T(coNum="CO1", course=course, plo=plolist[8])) #chnage
colist.append(CO_T(coNum="CO2", course=course, plo=plolist[9])) #chnage
colist.append(CO_T(coNum="CO3", course=course, plo=plolist[10])) #chnage
colist.append(CO_T(coNum="CO4", course=course, plo=plolist[11])) #chnage
...
faculties = []
faculties.append(Instructor_T.objects.get(pk=4401)) # change: faculty
faculties.append(Instructor_T.objects.get(pk=4402)) # change: faculty
faculties.append(Instructor_T.objects.get(pk=4403)) # change: faculty

dept = Department_T.objects.get(pk="MIS") # change
program = Program_T.objects.get(pk=4) #change
...
def updatedatabase(d, sem, y):
    ...
    df = pd.read_excel("MIS430.xlsx", sheet_name="Marks") # change
