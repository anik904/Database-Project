...
course = Course_T(courseID='ELT503', courseName="English3", numOfCredits=3, programID=5,
                  courseType="Core") #change
course.save()

# CO
plolist = list(PLO_T.objects.filter(programID=5))
...
faculties = []
faculties.append(Instructor_T.objects.get(pk=4501)) # change: faculty
faculties.append(Instructor_T.objects.get(pk=4502)) # change: faculty
faculties.append(Instructor_T.objects.get(pk=4503)) # change: faculty

dept = Department_T.objects.get(pk="ENG") # change
program = Program_T.objects.get(pk=5) #change
...
def updatedatabase(d, sem, y):
    ...
    df = pd.read_excel("ELT503.xlsx", sheet_name="Marks") # change
