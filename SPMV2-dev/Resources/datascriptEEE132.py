...
course = Course_T(courseID='EEE132', courseName="Introduction to Materials and Chemistry", numOfCredits=4, programID=3,
                  courseType="Foundation") #change
course.save()

# CO
plolist = list(PLO_T.objects.filter(programID=3))

colist = []

colist.append(CO_T(coNum="CO1", course=course, plo=plolist[4])) #chnage
colist.append(CO_T(coNum="CO2", course=course, plo=plolist[5])) #chnage
colist.append(CO_T(coNum="CO3", course=course, plo=plolist[6])) #chnage
colist.append(CO_T(coNum="CO4", course=course, plo=plolist[7])) #chnage
...
faculties = []
faculties.append(Instructor_T.objects.get(pk=4301)) # change: faculty
faculties.append(Instructor_T.objects.get(pk=4302)) # change: faculty
faculties.append(Instructor_T.objects.get(pk=4303)) # change: faculty

dept = Department_T.objects.get(pk="EEE") # change
program = Program_T.objects.get(pk=3) #change
...
def updatedatabase(d, sem, y):
    ...
    df = pd.read_excel("EEE132.xlsx", sheet_name="Marks") # change
