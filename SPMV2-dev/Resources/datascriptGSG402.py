...
course = Course_T(courseID='GSG402', courseName="GSG402", numOfCredits=3, programID=6,
                  courseType="Core") #change
course.save()

# CO
plolist = list(PLO_T.objects.filter(programID=6))
...
faculties = []
faculties.append(Instructor_T.objects.get(pk=4604)) # change: faculty
faculties.append(Instructor_T.objects.get(pk=4605)) # change: faculty
faculties.append(Instructor_T.objects.get(pk=4606)) # change: faculty

dept = Department_T.objects.get(pk="GSG") # change
program = Program_T.objects.get(pk=6) #change
...
def updatedatabase(d, sem, y):
    ...
    df = pd.read_excel("GSG402.xlsx", sheet_name="Marks") # change
